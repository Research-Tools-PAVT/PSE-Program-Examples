 ...
 * @param max_elem
 */
void make_pse_symbolic(void *addr, size_t bytes, const char *name,
                       long int min_elem, long int max_elem) {
  klee_make_symbolic(addr, bytes, name);
  klee_assume(*(long int *)addr >= min_elem);
  klee_assume(*(long int *)addr <= max_elem);
}