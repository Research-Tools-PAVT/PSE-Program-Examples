...
\begin{algorithm}
	...
	\label{alg:symbex}
	\begin{algorithmic}[1]
		\Function{SymbEx}{$S : Program$}
		\Let{$\varphi_{paths}$}{[]}, {$E_{s}$} $\leftarrow$ {[]}, $\sigma \leftarrow$ {[]}, $\varphi \leftarrow \emptyset$, $P \leftarrow \{ \}$  \algorithmiccomment{Initialization}
		\Let{$I_{0}$}{\textsc{getStartInstruction}($S$)}
		\State {$E_{s} \leftarrow$  \{[$I_{0}$, $\varphi$, $\sigma$, $P$]\}} \algorithmiccomment{Start State [$I_{0}$, $\varphi$, $\sigma$, $P$]}
		\While{$E_{s} \neq \emptyset$}
		\Let{$\Delta_{c}$}{\textsc{GetNext}($E_{s}$)}, $\Delta_{c}$ $\rightarrow$ [$I_{c}$, $\varphi$, $\sigma$, $P$]  \algorithmiccomment{Get Next state from $E_{s}$}
		\Switch{\textsc{instType}($I_{c}$)} \label{line:inst_case}
		\Case{$\mathtt{x} \xleftarrow{\$} d$} \algorithmiccomment{Create \texttt{fresh} Symbolic Variable} \label{line:make_pse}
		\Let{$I_{s}$}{\textsc{getNextInstruction}($I_{c}$)}
		\Let{$\varphi, \sigma, P$}{{\textbf{PSESample}}({$x,  d$, $\varphi$, $\sigma$, $P$})}
		\State {$E_{s} \leftarrow E_{s} \cup$ \{[$I_{c}$, $\varphi$, $\sigma$, $P$]\} }
		\EndCase
		\Case{$\mathtt{x} \leftarrow e$} \algorithmiccomment{Assignment Instruction} \label{line:assign}
		\Let{$I_{s}$}{\textsc{getNextInstruction}($I_{c}$)}
		\Let{$\varphi, \sigma, P$}{{\textbf{PSEAssignment}}({v, e, $\varphi$, $\sigma$, $P$})}
		\State {$E_{s} \leftarrow E_{s} \cup$ \{[$I_{c}$, $\varphi$, $\sigma$, $P$]\} }
		\EndCase
		\Case{\textbf{if} $(c_{sym})$ \textbf{then} $S_{1}$ \textbf{else} $S_{2}$} \algorithmiccomment{Branch Instruction}
		\Let{$((\varphi \wedge c_{sym}, \sigma,P,p_c), (\varphi \wedge \neg c_{sym}, \sigma,P,1-p_c))$}{{\textbf{PSEBranch}}({$c_{sym}$, $\varphi$, $\sigma$, $P$})} \label{line:branch}
		\Let{$I_{t}$}{\textsc{getStartInstruction}($P_{1}$)} \Let{$I_{f}$} {\textsc{getStartInstruction}($P_{2}$)}	
		\State {$E_{s} \leftarrow E_{s} \cup$ \{[$I_{t}$, $\varphi \wedge c_{sym}$, $\sigma$, $P$]\} }
		\State {$E_{s} \leftarrow E_{s} \cup$ \{[$I_{f}$, $\varphi \wedge \neg c_{sym}$, $\sigma$, $P$]\} }
		\EndCase
		\Case{\textbf{HALT}} \algorithmiccomment{Terminate Instruction}
		\Let{$\varphi_{paths}$}{$\varphi_{paths}$.\textsc{Append}($\varphi$)}		
		\EndCase
		\EndSwitch
		\EndWhile \\
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Let{$\sigma$}{Generate a fresh probabilistic symbolic variable}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Let{$c_{sym}$}{$\sigma[c]$}
		\Let{$(\sigma_1,\ldots,\sigma_n)$}{$\dom{P}$}
		\Let{$(d_1,\ldots,d_n)$}{$(P[\sigma_1],\ldots,P[\sigma_n])$}
		\Let{$p_c$}{$\frac{\displaystyle\sum_{(v_1,\ldots,v_n) \in \dom{d_1} \times \cdots\times \dom{d_n}} [(c_{sym} \wedge \varphi)\{\sigma_1 \mapsto v_1,\ldots,\sigma_n \mapsto v_n\}]}{\displaystyle\sum_{(v_1,\ldots,v_n) \in \dom{d_1} \times \cdots\times \dom{d_n}}[\varphi\{\sigma_1 \mapsto v_1,\ldots,\sigma_n \mapsto v_n\}]}$}
