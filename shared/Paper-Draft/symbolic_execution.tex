...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Let{$\varphi_{paths}$}{[]}, {$E_s$} $\leftarrow$ {[]}, $Enc_t \gets \bot, Enc_a \gets \bot$  \algorithmiccomment{Initialization}
		...
		\While{$L \neq \phi$}
		\Let{($I_{c}, \varphi, \sigma, P, p)$}{$E_s$.\textsc{Remove}( )}
		\If{\textsc{UNSAT}($\varphi$) $\lor (p = 0.0)$}
		\State \textbf{continue}
		\EndIf
		\Switch{\textsc{instType}($I_{c}$)} \label{line:inst_type}
		\Case{\fbox{$\mathtt{x} \xleftarrow{\$} d$}} \algorithmiccomment{Create \texttt{New} Symbolic Variable} \label{line:pse_sampling}
		\Let{$I_1$}{\textsc{getNextInstruction}($I_{c}$)}
		\State{\colorbox[RGB]{200, 200, 200}{$\sigma,~P_1 \leftarrow~${\textbf{PSESample}}~({$x,~d,~P$})}}
		\Let{$S_1$}{($I_{c}, \varphi, \sigma, P_1, p$)}
		\State {$E_s$.\textsc{Append}({$S_1$})}
		\EndCase
		\Case{\fbox{$x \gets e$}} \algorithmiccomment{Assignment Instruction} \label{line:symbex_assign}
		\Let{$I_1$}{\textsc{getNextInstruction}($I_{c}$)}
		%\Let{$e_{sym}$}{$\sigma[e]$}
		%\Let{$\varphi, \sigma, P$}{{\textbf{PSEAssignment}}({v, e, $\varphi$, $\sigma$, $P$})}
		\Let{$\sigma[x]$}{$\sigma[e]$}
		\Let{$S_{1}$}{($I_{1}$, $\varphi$, $\sigma$, $P$, $p$)}
		\State {$E_s$.\textsc{Append}({$S_{1}$})}
		\EndCase
		\Case{\fbox{\textbf{if} $(c)$ \textbf{then goto} $T_{1}$}} \algorithmiccomment{Branch Instruction}
		\Let{$c_{sym}$}{$\sigma[c]$}
		\State{\colorbox[RGB]{200, 200, 200}{$p_t,~p_f \leftarrow~${\textbf{PSEBranch}}~({$c_{sym},~\sigma,~P$})}}
		\Let{$I_{1}$}{\textsc{getInstruction}($T_{1}$)} 
		\Let{$I_{2}$} {\textsc{getNextInstruction}(I)}	
		\Let{$S_{t}$}{($I_{1}$, $\varphi \wedge c_{sym}$, $\sigma$, $P$, $p * p_t$)} \label{line:symbex_true_state} \algorithmiccomment{Multiply with previous \textit{probability} value.}
		\Let{$S_{f}$}{($I_{2}$, $\varphi \wedge \neg c_{sym}$, $\sigma$, $P$, $p * p_f$)} \label{line:symbex_false_state}
		\State {$E_s$.\textsc{Append}({$S_{f}$})}
		\State {$E_s$.\textsc{Append}({$S_{t}$})} \algorithmiccomment{Start with True State}
		\EndCase
		\Case{\fbox{\textbf{halt(c)}}} \algorithmiccomment{Terminate Instruction}
		\Let{$c_{sym}$}{$\sigma[c]$}
		\If{\textsc{SAT}($c_{sym}$)}
		\State {\colorbox[RGB]{200, 200, 200}{$Enc_t$.\textsc{AddEncoding}~($\varphi,~p$)}}		
		\EndIf
		\State {\colorbox[RGB]{200, 200, 200}{$Enc_a$.\textsc{AddEncoding}~($\varphi,~p$)}}		
		\EndCase
		\EndSwitch
		\EndWhile 
		\Let{$r$}{\colorbox[RGB]{200, 200, 200}{\textsc{Solve}~($Enc_t,~Enc_a$)}} \\
		\Return{$r$} 
		\EndFunction
	\end{algorithmic}
\end{algorithm}
