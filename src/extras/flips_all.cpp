...
int main(int argc, char *argv[]) {
  ...
  make_pse_symbolic(&TOTAL_SUM, sizeof(TOTAL_SUM), "TOTAL_SUM", 0,
                    COINS * FLIPS * LIMIT);

  // klee_assume(b1 >= 1 && b1 <= LIMIT);
  // klee_assume(b2 >= 1 && b2 <= LIMIT);

  for (b1 = 1; b1 <= LIMIT; b1++) {
    for (b2 = 1; b2 <= LIMIT; b2++) {
      int sum1 = 0, sum2 = 0, SUM, tmp1[FLIPS], tmp2[FLIPS];
      ...
      for (std::size_t i = 0; i < FLIPS; ++i) {
        ...
        // make_pse_symbolic(&coin_curr1, sizeof(coin_curr1),
        // outcome_str.c_str(),
        //                   (int)0, (int)1);
        tmp1[i] = temp1;
        if (temp1 >= b1)
      ...
      for (std::size_t i = 0; i < FLIPS; ++i) {
        ...
        // make_pse_symbolic(&coin_curr2, sizeof(coin_curr2),
        // outcome_str.c_str(),
        //                   (int)0, (int)1);
        tmp2[i] = temp2;
        if (temp2 >= b2)
  ...
  TOTAL_SUM = (TOTAL_SUM / (LIMIT * LIMIT));
  klee_dump_kquery_state();
  mark_state_winning();
  expected_value("TOTAL_SUM", TOTAL_SUM);
  return 0;
}
